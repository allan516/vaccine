  ...
  try {
    ...
    if (vaccineExisting) {
      vaccineExisting.vaccines.forEach((value) => {
        ...
        if (value.id.toString() === vaccineId.toString() && !vaccine.date) {
          throw new Error(
            "Para agendar uma vacina é necessário agendar para uma data válida."
          );
        }
        ...
        if (
          vaccineId.toString() === value.id.toString() &&
          vaccine.date !== value.date &&
          !validate(vaccine)
        ) {
          throw new Error(
            `Data inválida ${vaccine.date}, não é possível reagendar a vacina para uma data no passado.`
          );
        }
        if (
          value.id.toString() === vaccineId.toString() &&
